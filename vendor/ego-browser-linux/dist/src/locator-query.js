...
function textElementsExpression(locator, rootExpression = "document") {
  const query = rootExpression === "document" ? "document.querySelectorAll('body *')" : `${rootExpression}.querySelectorAll('*')`;
  const match = textMatchExpression(
    "el.innerText || el.textContent",
    locator.text,
    locator.exact
...
function parseRoleLocator(value) {
  ...
  return {
    role: roleMatch[1],
    name: roleMatch[2] === void 0 ? void 0 : parseLocatorMatcher(roleMatch[2])
  };
}
