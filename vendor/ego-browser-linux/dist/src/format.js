...
function formatCliLogValue(value) {
  ...
  return JSON.stringify(toLoggable(value, [], /* @__PURE__ */ new WeakSet()), null, 2);
}
function toLoggable(value, path, stack) {
  ...
  try {
    if (Array.isArray(value)) {
      return value.map(
        (item, index) => toLoggable(item, [...path, String(index)], stack)
      );
    }
    const out = {};
...
function functionLogValue(fn, path) {
  ...
  const doc = key ? FUNCTION_DOCS[key] : void 0;
