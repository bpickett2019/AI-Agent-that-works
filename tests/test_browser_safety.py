...
class ViewerSafetyTests(unittest.TestCase):
    ...
    def test_three_user_operation_switcher_maps_only_active_worker_slots(self):
        ...
        self.assertIn('jobAtStart?await request(`/api/status?job_id=',HTML)
        self.assertNotIn('`/api/status?worker_slot=${workerAtStart}`',HTML)
        ...
        self.assertIn('No exact authorized match.',HTML)
        self.assertIn('Authorized event list unavailable:',HTML)
...
class BrowserTargetSafetyTests(unittest.TestCase):
    ...
    def test_ego_inside_steel_is_the_only_active_router(self):
        ...
        self.assertLess(EGO_DIRECT.index('let writesAttempted=0,actionIndex=-1'), EGO_DIRECT.index('try{\n  const ego='))
