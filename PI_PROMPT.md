...
- Before any write, open Edit in a READ-ONLY round and identify every target plus the exact Save control. Do not fill a field merely to probe it. A write round is prevalidated as one atomic change → Save → wait → fresh full snapshot/readback; a missing commit path is an internal planning error with ZERO writes, not an item hold.
- Write scripts use sequential awaited Page actions with literal exact observed locators, literal verified desired values (UI date formatting is allowed), and explicit `{rrSource: "the exact source string from cvent_plan"}` on EACH data action. The header `rrSources` contains their exact verified sources. Do not use a previous field's cell. Save, Tab, Edit, scrolling and navigation have no independent rrSource. Use `page.keyboard.press("Tab")` and `page.click(the literal Save locator)` without fake cell evidence.
- Begin the write script with `console.log(await page.snapshot({scope:"full_page"}))`; fill/select all mapped differences; click the known Save; `await page.waitForLoadState()`; print a fresh full `page.snapshot()` for persisted-value verification. Compare every written value with this readback and record it through cvent_verify_domain. A snapshot of an unsaved editor is not persisted verification.
- Keep conditional discovery and snapshot/ref parsing in read-only rounds. Once controls are resolved, emit straight-line, awaited literal actions for the atomic write round, not conditional Save, computed methods, loops or dynamic targets. If validation rejects a round, repair the whole round before retrying; do not split off a field-only write. No unrestricted page.evaluate/CDP is available: use semantic snapshots and audited native actions.
