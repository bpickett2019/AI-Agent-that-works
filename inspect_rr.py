...
for ws in wb.worksheets:
    ...
    result["sheets"].append({"name":ws.title,"max_row":ws.max_row,"max_column":ws.max_column,
...
summary={"workbook":str(p),"full_inspection":str(out),"sheets":[]}
for sheet in result['sheets']:
    rows=sheet['populated_rows']; samples=rows[:12]+(rows[-3:] if len(rows)>15 else [])
    summary['sheets'].append({'name':sheet['name'],'max_row':sheet['max_row'],'max_column':sheet['max_column'],
