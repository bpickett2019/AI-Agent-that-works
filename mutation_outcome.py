...
def mutation_outcome(directory: Path) -> dict:
    ...
    attempted, succeeded, rejected, not_persisted = Counter(), Counter(), Counter(), Counter()
    malformed = False
    resolutions = {}
    ...
    if resolution_file.exists():
        try:
            for resolution in json.loads(resolution_file.read_text())['resolutions']:
                ...
                if (resolution['outcome'] != 'NOT_PERSISTED' or resolution['actor'] != 'operator'
        ...
        for line in audit.read_text(errors="replace").splitlines():
            try:
                ...
                if item.get("result") == "attempted":
                    attempted[key] += 1
                    if (item.get('at'), *key) in resolutions: not_persisted[key] += 1
    ...
    unmatched = sum(max(0, count - succeeded[key] - not_persisted[key]) for key, count in effective.items())
    ...
    return {
        ...
        "notPersisted": sum(not_persisted.values()), "hasAttempts": has_attempts, "uncertainMarker": uncertain_marker.exists(),
